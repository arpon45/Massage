...
export default function CallDashboard() {
  ...
  const [nav, setNav] = React.useState(pathToNav[router.pathname] !== undefined ? pathToNav[router.pathname] : 2);
  const [drawerOpen, setDrawerOpen] = React.useState(false);
  const [selectedCall, setSelectedCall] = useState<string | undefined>(undefined);
  const calls: import('./CallList').CallItem[] = [
    { id: '1', name: 'Alice', avatarUrl: '', type: 'voice', time: '09:30', missed: false },
    { id: '2', name: 'Bob', avatarUrl: '', type: 'video', time: 'Yesterday', missed: true },
    { id: '3', name: 'Charlie', avatarUrl: '', type: 'voice', time: 'Yesterday', missed: false },
    { id: '4', name: 'Diana', avatarUrl: '', type: 'video', time: '2 days ago', missed: true }
  ];
        ...
        <Paper elevation={0} sx={{
          ...
          {calls.length === 0 ? (
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 180, py: 4 }}>
              <Typography component="div" sx={{ fontWeight: 600, color: '#757575', fontSize: 22, mb: 0.5 }}>No calls found</Typography>
              <Typography component="div" sx={{ color: '#9e9e9e', fontSize: 15 }}>Your recent calls will appear here.</Typography>
            </Box>
          ) : (
            <CallList calls={calls} onSelect={setSelectedCall} selectedId={selectedCall} />
          )}
        </Paper>
      </Box>
      {/* Drawer for menu */}
